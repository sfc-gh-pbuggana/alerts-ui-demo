...
import { alertCategories, defaultRoles, defaultUserOptions, defaultExternalDestinations } from "@/lib/alert-config"
import { getSQLTemplate } from "@/lib/sql-templates"
import "@/styles/alerts.css"

// Dynamic icon rendering
const renderCategoryIcon = (iconName: string, className?: string) => {
  const iconProps = { className: className || "text-blue-600" }
...
export default function DynamicAlertPage({ params }: AlertPageProps) {
  const resolvedParams = use(params)
  const searchParams = useSearchParams()
  
  // Find the category configuration
  const categoryConfig = alertCategories.find(cat => cat.id === resolvedParams.category)
    ...
    return (
      <div className="flex h-screen bg-gray-50">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <h1 className="text-2xl font-bold text-gray-900">Alert Category Not Found</h1>
          <p className="text-gray-600 mt-2">The requested alert category does not exist.</p>
          <Link href="/alerts">
  ...
  // Custom config state (for templates with special configuration)
  ...
  const handleSaveAlerts = () => {
    ...
    setTimeout(() => {
      ...
      console.log(`Saving ${categoryConfig.name} alerts...`)
        ...
        <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-[var(--border)] bg-[var(--panel)]">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <nav className="text-sm text-[var(--subtle-text)] truncate">
              <Link href="/alerts" className="text-[var(--text)] font-medium hover:text-blue-600">Alert Center</Link>
              <span className="mx-2 text-[var(--subtle-text)]">/</span>
              <span className="text-[var(--text)]">{categoryConfig.name} Alerts</span>
            </nav>
          </div>
        ...
        <main className="flex-1 overflow-y-auto max-w-6xl mx-auto p-6">
          <div className="mb-8 flex items-center gap-4">
            <div className="flex items-center justify-center w-12 h-12 rounded-lg bg-[var(--panel-3)] border border-[var(--border)]">
              {renderCategoryIcon(categoryConfig.icon, "text-blue-600")}
            </div>
            <div>
              <h1 className="text-3xl font-bold text-[var(--text)] mb-2">
                {categoryConfig.name} Alert Configuration
              </h1>
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {categoryConfig.templates.map((template) => {
                    ...
                    >
                      <div className="flex items-start space-x-3 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          {/* Custom configuration sections can be added here based on template.customConfig */}
                        </div>
                      </div>
            ...
            <NotificationChannels
              ...
              selectedEmailRoles={selectedEmailRoles}
              emailRolesExpanded={emailRolesExpanded}
              emailUsersExpanded={emailUsersExpanded}
              setEmailRolesExpanded={setEmailRolesExpanded}
              setEmailUsersExpanded={setEmailUsersExpanded}
              addEmailUser={addEmailUser}
              removeEmailUser={removeEmailUser}
              addEmailRole={addEmailRole}
              removeEmailRole={removeEmailRole}
              inAppUsers={inAppUsers}
              selectedInAppRoles={selectedInAppRoles}
              inAppExpanded={inAppExpanded}
              inAppRolesExpanded={inAppRolesExpanded}
              inAppUsersExpanded={inAppUsersExpanded}
              setInAppExpanded={setInAppExpanded}
              setInAppRolesExpanded={setInAppRolesExpanded}
              setInAppUsersExpanded={setInAppUsersExpanded}
              addInAppUser={addInAppUser}
              removeInAppUser={removeInAppUser}
              addInAppRole={addInAppRole}
              removeInAppRole={removeInAppRole}
              externalDestinations={externalDestinations}
              externalExpanded={externalExpanded}
              setExternalExpanded={setExternalExpanded}
              addExternalDestination={addExternalDestination}
              removeExternalDestination={removeExternalDestination}
              roles={defaultRoles}
              userOptions={defaultUserOptions}
              externalOptions={defaultExternalDestinations}
            />
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Select defaultValue="1">
                      <SelectTrigger className="w-20 h-9 border-gray-300 bg-gray-50 text-gray-900">
                        <SelectValue placeholder="1" />
                      </SelectTrigger>
                      <SelectContent className="z-50 bg-white text-gray-900 border-gray-300 shadow-md">
                        {["1", "2", "3", "4", "6", "12"].map((v) => (
                    ...
                    <Select defaultValue="day">
                      <SelectTrigger className="w-36 h-9 border-gray-300 bg-gray-50 text-gray-900">
                  ...
                  <Select defaultValue="0">
                    <SelectTrigger className="w-64 h-9 border-gray-300 bg-gray-50 text-gray-900">
                  ...
                  <Select defaultValue="pt">
                    <SelectTrigger className="w-[300px] h-9 border-gray-300 bg-gray-50 text-gray-900">
            ...
            <div className="flex justify-end gap-4 pt-8 border-t border-gray-200">
              <Link href="/alerts">
                <Button 
                  variant="outline" 
                  className="px-6 py-2 border-blue-600 bg-white text-blue-600 hover:bg-blue-600 hover:text-white font-medium"
                  disabled={isSaving}
                >
                  Cancel
                </Button>
        ...
        <DialogContent className="max-w-md bg-white border border-gray-200">
          <DialogHeader className="text-center">
            ...
            <DialogDescription className="text-gray-600 mt-2">
              Your {categoryConfig.name} alert templates have been saved and activated. You should receive test notifications in each of your specified channels within the next few minutes.
            </DialogDescription>
          </DialogHeader>
